...
- File organization uses a transactional isolate-copy, backup, full-scope organize, reference-repair, Git-layout restore, isolated cleanup, validated replacement, active-tree cleanup, real-global revalidation, regenerated-cache cleanup, and lifecycle-writeback loop. The default scope is every non-backup, non-protected file under the project root, not only `00-inbox`; `.git`, `.codex`, credentials, runtimes, dependency trees, and external backup roots remain excluded. Each cleanup pass re-evaluates the current tree and deletes only untracked allowlisted temporary/cache artifacts plus truly empty directories. Every file is copied to an off-root quarantine and hash-verified first; stale pre-validation hashes are never reused, and tracked files survive even when their names look disposable. After replacement, the active repository must pass validation twice, remove validation-regenerated caches, and pass its real global interfaces before aggregate evidence and lifecycle state are written back. The Git gate accepts only this complete proof. See [File Organization Architecture](docs/assets/file-organization-architecture.mmd) and [image provenance](docs/assets/file-organization-concept.provenance.md).
