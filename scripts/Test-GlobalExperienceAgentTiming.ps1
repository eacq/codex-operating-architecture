...
foreach ($term in @('TaskStartedAt','HostWorkedSeconds','ClientTimingJson','ExternalElapsedSeconds','ScreenshotCaptureSeconds')) {
    if ($controller -notmatch [regex]::Escape($term) -or $runtime -notmatch [regex]::Escape($term)) { throw "GEA timing input missing: $term" }
}
foreach ($term in @('task_wall_clock_seconds','customer_visible_complete_seconds','customer_visible_time_source','client_timing','external_monotonic_seconds','screenshot_capture_seconds','cross_validation_status','host_reported_worked_seconds','operation_wall_clock_seconds','agent_session_wall_clock_seconds','task_time_status')) {
