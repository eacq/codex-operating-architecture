...
$sandbox = Join-Path $backupRoot "iterations\$stamp"
...
function Mount-ValidationRuntimeOverlay {
  $mounts = @(
    [pscustomobject]@{
      source = Join-Path $root '.runtime\work\network-learning-ppt-master'
      target = Join-Path $sandbox '.runtime\work\network-learning-ppt-master'
    },
    [pscustomobject]@{
      source = Join-Path $root '.runtime\work\network-learning\ppt-master'
      target = Join-Path $sandbox '.runtime\work\network-learning\ppt-master'
    ...
    [pscustomobject]@{
      source = Join-Path $root '.runtime\work\agent-ppt-decks'
      target = Join-Path $sandbox '.runtime\work\agent-ppt-decks'
    },
    [pscustomobject]@{
  ...
  foreach ($mount in $mounts) {
    if (-not (Test-Path -LiteralPath $mount.source -PathType Container)) {
      throw "Required validation runtime source is missing: $($mount.source)"
    }
    $sourceFull = [IO.Path]::GetFullPath($mount.source)
    $targetFull = [IO.Path]::GetFullPath($mount.target)
    if (-not $targetFull.StartsWith($sandbox + '\', [StringComparison]::OrdinalIgnoreCase)) {
      throw "Validation runtime mount escaped sandbox: $($mount.target)"
    }
    if (Test-Path -LiteralPath $mount.target) {
      throw "Validation runtime mount target already exists: $($mount.target)"
    }
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $mount.target) | Out-Null
    New-Item -ItemType Junction -Path $mount.target -Target $sourceFull | Out-Null
  }
  return @($mounts)
}
function Dismount-ValidationRuntimeOverlay([object[]]$Mounts) {
  foreach ($mount in @($Mounts)) {
    if (-not $mount) { continue }
    $targetFull = [IO.Path]::GetFullPath([string]$mount.target)
    if (-not $targetFull.StartsWith($sandbox + '\', [StringComparison]::OrdinalIgnoreCase)) {
      throw "Validation runtime unmount escaped sandbox: $($mount.target)"
    }
    if (Test-Path -LiteralPath $mount.target) {
      $item = Get-Item -LiteralPath $mount.target -Force
      if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
        Remove-Item -LiteralPath $mount.target -Force
      } else {
        throw "Validation runtime mount target is not a junction: $($mount.target)"
      }
    }
  }
}
...
Invoke-Step 'clone sandbox' {
  ...
  $trackedFiles = (& git -C $root ls-files) -join [Environment]::NewLine
  [IO.File]::WriteAllText($trackedAuthority, ($trackedFiles + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
  & git -C $sandbox add -- .
...
$script:validationRuntimeRootPrevious = [Environment]::GetEnvironmentVariable('CODEX_VALIDATION_RUNTIME_ROOT', 'Process')
$script:validationRuntimeRootSet = $false
Invoke-Step 'validate sandbox' {
  [Environment]::SetEnvironmentVariable('CODEX_VALIDATION_RUNTIME_ROOT', $root, 'Process')
  $script:validationRuntimeRootSet = $true
...
Invoke-Step 'restore validation runtime environment' {
  if ($null -eq $script:validationRuntimeRootPrevious) {
    Remove-Item Env:CODEX_VALIDATION_RUNTIME_ROOT -ErrorAction SilentlyContinue
  } else {
    [Environment]::SetEnvironmentVariable('CODEX_VALIDATION_RUNTIME_ROOT', $script:validationRuntimeRootPrevious, 'Process')
  }
  $script:validationRuntimeRootSet = $false
}
...
[IO.File]::WriteAllText($proofPath, (($result | ConvertTo-Json -Depth 8) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
if ($Replace) {
  ...
  } | ForEach-Object { [IO.File]::WriteAllText((Join-Path $root '.codex\project\file-organization-review.json'), (($_ | ConvertTo-Json -Depth 6) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false)) }
...
$result | ConvertTo-Json
  ...
  if ($script:validationRuntimeRootSet) {
    if ($null -eq $script:validationRuntimeRootPrevious) {
      Remove-Item Env:CODEX_VALIDATION_RUNTIME_ROOT -ErrorAction SilentlyContinue
    } else {
      [Environment]::SetEnvironmentVariable('CODEX_VALIDATION_RUNTIME_ROOT', $script:validationRuntimeRootPrevious, 'Process')
  ...
  $failurePath = Join-Path $root '.codex\project\last-iteration-failure.json'
  [IO.File]::WriteAllText($failurePath, (($failureRecord | ConvertTo-Json -Depth 5) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
  if (Test-Path -LiteralPath $statePath) {
    $state = Get-Content -LiteralPath $statePath -Raw -Encoding UTF8 | ConvertFrom-Json
    $state.updated_at = [DateTime]::Now.ToString('o')
