...
foreach ($migration in @($policy.migrations)) {
    ...
    if ($migration.status -eq 'retired') {
        if (Test-Path -LiteralPath (Join-Path $legacyRoot 'SKILL.md') -PathType Leaf) { throw "Retired compatibility skill still has a discovery entry: $($migration.from)" }
        continue
    }
