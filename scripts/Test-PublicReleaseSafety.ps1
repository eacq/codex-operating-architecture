...
function Test-ForbiddenPublicPath([string]$Path) {
    if ($Path -match '(^|/)(\.codex|\.runtime|\.history-cache|\.sandbox-secrets|private-skill-config)(/|$)') { return $true }
    if ($Path -match '(^|/)(auth\.json$|.*\.dpapi$)') { return $true }
...
if (@($paths | Where-Object { Test-ForbiddenPublicPath $_ }).Count -gt 0) { throw 'Public release snapshot contains a forbidden private-state path.' }
...
[pscustomobject]@{
    ...
    public_repository = $publicRepo
    private_repository_checked = -not [bool]$SkipRemoteIdentityCheck
    result = 'public-release-safety-passed'
} | ConvertTo-Json
