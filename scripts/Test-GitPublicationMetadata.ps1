...
if ($presentationScope.Count -gt 0) {
    ...
    $auditContent = if ($changed -contains $auditPath) {
        (& git -C $root show ":$auditPath") -join "`n"
    } else {
        $auditFullPath = Join-Path $root $auditPath
        if (-not (Test-Path -LiteralPath $auditFullPath)) { throw "Main user-facing explanation changes require the presentation audit: $auditPath" }
        Get-Content -LiteralPath $auditFullPath -Raw -Encoding UTF8
    }
