...
foreach ($candidate in $pythonCandidates) {
    ...
    & $candidate -s -c "import yaml" 2>$null
...
$skillNames = @(Get-ChildItem -LiteralPath (Join-Path $root 'skills') -Directory | Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName 'SKILL.md') -PathType Leaf } | ForEach-Object Name | Sort-Object)
...
if ($configuredPython -and (Test-Path -LiteralPath $configuredPython) -and $configuredPython.StartsWith($root, [StringComparison]::OrdinalIgnoreCase)) {
    & $configuredPython -s -c "import yaml, docx, pptx, pypdf, PIL, lxml, xlsxwriter"
    if ($LASTEXITCODE -ne 0) { throw 'Configured project Python is not self-contained for validation and Office/media imports.' }
}
...
& (Join-Path $root 'skills\codex-image-workflow\subskills\figure-optimization\scripts\Test-ScientificFigureWorkflow.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Scientific figure workflow test failed.' }
...
& (Join-Path $root 'skills\codex-file-organization\scripts\Test-FileOrganizationNoMoveFastPath.ps1')
if (-not $?) { throw 'File-organization no-move fast-path test failed.' }
& (Join-Path $root 'skills\codex-file-organization\scripts\Test-FileOrganizationGlobalIterationScope.ps1')
if (-not $?) { throw 'File-organization global-iteration scope test failed.' }
& (Join-Path $root 'skills\codex-file-organization\scripts\Test-FileOrganizationBackupGitFastPath.ps1')
if (-not $?) { throw 'File-organization Git backup fast-path test failed.' }
...
& (Join-Path $root 'skills\codex-learning\scripts\Test-HistoricalLearningReconciliation.ps1') -ProjectRoot $root
...
& (Join-Path $root 'skills\codex-git-operations\scripts\Test-GitHubNetworkCommandArgumentBinding.ps1')
if (-not $?) { throw 'GitHub network command argument-binding test failed.' }
...
& (Join-Path $root 'skills\codex-git-operations\scripts\Test-ExperienceReleasePublicationResume.ps1')
if (-not $?) { throw 'Experience release publication resume test failed.' }
& (Join-Path $root 'skills\codex-git-operations\scripts\Test-ExperienceChangeCloseout.ps1')
if (-not $?) { throw 'Experience change closeout wrapper test failed.' }
...
& (Join-Path $root 'scripts\Test-SkillTerminology.ps1')
if (-not $?) { throw 'Skill terminology contract test failed.' }
& (Join-Path $root 'scripts\Test-AgentHarnessContract.ps1')
if (-not $?) { throw 'Agent harness contract test failed.' }
& (Join-Path $root 'scripts\Test-AgentSystemTopology.ps1')
& (Join-Path $root 'scripts\Test-AgentStructuralOptimizationPropagation.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Agent system topology test failed.' }
& (Join-Path $root 'scripts\Test-AgentOwnerConnections.ps1')
if (-not $?) { throw 'Agent owner connection test failed.' }
& (Join-Path $root 'scripts\Test-AgentInterfacePolicy.ps1')
if (-not $?) { throw 'Agent interface policy test failed.' }
& (Join-Path $root 'scripts\Test-AgentIntentRecognition.ps1')
if (-not $?) { throw 'Agent intent recognition test failed.' }
& (Join-Path $root 'scripts\Test-AgentLoopPolicy.ps1')
if (-not $?) { throw 'Agent loop policy test failed.' }
& (Join-Path $root 'scripts\Test-AgentCodebaseMemoryPolicy.ps1')
if (-not $?) { throw 'Agent Codebase Memory policy test failed.' }
& (Join-Path $root 'scripts\Test-AgentChunkingRetrieval.ps1')
if (-not $?) { throw 'Agent chunking retrieval test failed.' }
& (Join-Path $root 'scripts\Test-AgentDocParsePipeline.ps1')
if (-not $?) { throw 'Agent document parse pipeline test failed.' }
& (Join-Path $root 'skills\codex-office-cli\subskills\ppt-deck-factory\scripts\Test-AgentPptDeckFactory.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Agent PPT deck factory test failed.' }
& (Join-Path $root 'scripts\Test-AgentCareerPlanningAgent.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Agent career planning child test failed.' }
& (Join-Path $root 'agent\80-maintenance\Test-AgentFilesystem.ps1')
if (-not $?) { throw 'Agent filesystem projection test failed.' }
& (Join-Path $root 'scripts\Test-GlobalExperienceAgentRegistry.ps1')
if (-not $?) { throw 'Global experience Agent registry test failed.' }
& (Join-Path $root 'scripts\Test-GlobalExperienceAgent.ps1')
if (-not $?) { throw 'Global experience agent test failed.' }
& (Join-Path $root 'scripts\Test-AgentMemorySystem.ps1')
if (-not $?) { throw 'Agent memory system test failed.' }
& (Join-Path $root 'scripts\Test-AgentMemoryAsSkillPolicy.ps1')
if (-not $?) { throw 'Agent memory-as-skill policy test failed.' }
& (Join-Path $root 'scripts\Test-AgentSkillInvocationPolicy.ps1')
if (-not $?) { throw 'Agent skill invocation policy test failed.' }
& (Join-Path $root 'scripts\Test-AgentArchitectureDiagram.ps1')
if (-not $?) { throw 'Agent architecture diagram test failed.' }
...
& (Join-Path $root 'skills\codex-architecture-iteration\scripts\Test-UnitTopologyReview.ps1')
if (-not $?) { throw 'Unit topology review test failed.' }
& (Join-Path $root 'skills\codex-architecture-iteration\scripts\Test-DeepArchitectureAudit.ps1')
if (-not $?) { throw 'Deep architecture audit test failed.' }
& (Join-Path $root 'skills\codex-self-evolution\subskills\resource-economy\Test-ResourceModePolicy.ps1')
if (-not $?) { throw 'Resource mode policy test failed.' }
& (Join-Path $root 'scripts\Test-LoopXTrajectory.ps1')
if (-not $?) { throw 'LoopX trajectory resource-version test failed.' }
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-ExperienceAgentAccess.ps1')
if (-not $?) { throw 'Experience Agent access test failed.' }
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-MinimalAgentTemplate.ps1')
if (-not $?) { throw 'Minimal Agent template test failed.' }
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-MethodologySkillDistillation.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Methodology skill distillation test failed.' }
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-SkillEvolutionOptimization.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Skill evolution optimization test failed.' }
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-ExperienceBookShelfLearning.ps1') -RepositoryRoot $root
if (-not $?) { throw 'Experience book shelf learning test failed.' }
...
& (Join-Path $root 'skills\codex-experience-capture\scripts\Test-CandidateAuthorizationInheritance.ps1')
& (Join-Path $root 'skills\codex-git-operations\scripts\Test-AutoCommitIterationIntegration.ps1')
if (-not $?) { throw 'Auto-commit iteration integration test failed.' }
& (Join-Path $root 'scripts\Test-RepositoryTextEncoding.ps1') -RepositoryRoot $root -IncludeAgentRuntimeState
if (-not $?) { throw 'Repository UTF-8 encoding contract test failed.' }
Write-Host 'All skills, history indexing, module registry, knowledge graph, mind-map views, image workflow tests, cleanup tests, rollback tests, continuous diagnosis tests, Git recovery tests, error-feedback tests, terminology, brainstorming and writing-plan contract tests, unit topology review, and global error-feedback inbox tests validated.'
