...
function Remove-JunctionInterface {
    param(
    ...
    if (-not $resolvedPath.StartsWith($resolvedRoot + '\', [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to remove global skill interface outside target root: $resolvedPath"
    }
    $item = Get-Item -LiteralPath $resolvedPath -Force
    if ($item.LinkType -ne 'Junction') {
        throw "Refusing to remove non-junction stale skill interface: $resolvedPath"
    }
    [System.IO.Directory]::Delete($resolvedPath, $false)
}

$sourceSkillDirectories = @(Get-ChildItem -LiteralPath $sourceRoot -Directory | Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName 'SKILL.md') -PathType Leaf })
...
Get-ChildItem -LiteralPath $targetRoot -Directory -Force | ForEach-Object {
    if ($sourceSkillNames -notcontains $_.Name -and $_.LinkType -eq 'Junction') {
        Remove-JunctionInterface -Path $_.FullName -ExpectedRoot $targetRoot
        Write-Host "Removed retired global skill interface $($_.Name)"
    }
}
