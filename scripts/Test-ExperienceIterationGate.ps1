...
if ($organizationProbe.result -ne 'completed' -or -not $organizationProbe.validated -or -not $organizationProbe.replaced -or -not $organizationProbe.post_replacement_validated -or -not $organizationProbe.lifecycle_written_back -or -not $organizationProbe.rollback_ready) { throw 'The isolated rollback, cleanup, replacement, post-validation, and lifecycle proof is incomplete.' }
