...
- During every Codex project entry, run `$ARCHITECTURE_ROOT\scripts\Start-CodebaseMemoryGraphUi.ps1` for `F:\codex` on `127.0.0.1:9750`; it blocks until the UI service itself has indexed the global experience system as `F-codex`. When `codebase-memory-mcp` is callable, also refresh the task-local evidence graph with `index_repository(repo_path="F:\\codex", mode="fast", name="F-codex")`, then refresh the current source repository when it differs. Treat `F-codex` as the global experience system's structural index; do not create aliases. If the MCP tool is unavailable in the current Codex task, record that fact and continue with normal local evidence gathering.
