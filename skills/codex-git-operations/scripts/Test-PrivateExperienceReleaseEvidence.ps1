...
if ($ApplyLifecycleState) {
    ...
    $state | Add-Member -NotePropertyName last_commit_version -NotePropertyValue $Version -Force
...
[ordered]@{
    ...
    scripted_steps = @('head', 'origin-main', 'local-tag', 'remote-tag', 'private-visibility', 'release-status', 'lifecycle-version-writeback')
