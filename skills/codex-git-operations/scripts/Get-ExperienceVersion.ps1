...
switch ($Action) {
    ...
    'AutoFeature' { $next = [pscustomobject]@{ First = $current.First; Second = $current.Second; Third = $current.Third + 1; Fourth = 0 } }
    'AutoFix' { $next = [pscustomobject]@{ First = $current.First; Second = $current.Second; Third = $current.Third; Fourth = $current.Fourth + 1 } }
