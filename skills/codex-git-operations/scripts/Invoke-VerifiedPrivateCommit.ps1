...
$iterationStatusReady = Test-Path -LiteralPath (Join-Path $root 'docs\ITERATION-STATUS.md')
$metadataReady = $changelogReady -and $hasDocs -and ((-not $hasImplementation) -or $iterationStatusReady) -and ((-not $hasVersion) -or $hasReleaseNote)
...
& (Join-Path $root 'scripts\Test-GitPublicationMetadata.ps1') -RepositoryRoot $root -Staged | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Publication metadata validation failed.' }
& git -C $root commit -m $Message | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Git commit failed.' }
$commit = (& git -C $root rev-parse HEAD).Trim()
if (-not $CommitOnly) {
    & $githubCommand -RepositoryRoot $root -ForceProxy:$ForceProxy -Tool git -C $root push origin HEAD | Out-Null
}
