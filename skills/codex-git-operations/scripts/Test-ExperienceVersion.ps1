...
$autoFix = & $calculator -Action AutoFix -RepositoryRoot $root -CurrentVersion '2.1.0.0' -PublicReleaseRows @('v2.0 Latest v2.0 2026-07-20') -PrivateReleaseRows @() -UseProvidedReleaseRows | ConvertFrom-Json
if ($autoFix.version -ne '2.1.0.1' -or $autoFix.release_tag -ne 'v2.1') { throw 'AutoFix did not preserve the current release components.' }
$autoFeature = & $calculator -Action AutoFeature -RepositoryRoot $root -CurrentVersion '2.1.0.1' -PublicReleaseRows @('v2.0 Latest v2.0 2026-07-20') -PrivateReleaseRows @() -UseProvidedReleaseRows | ConvertFrom-Json
if ($autoFeature.version -ne '2.1.1.0' -or $autoFeature.release_tag -ne 'v2.1') { throw 'AutoFeature did not preserve the current release components.' }
