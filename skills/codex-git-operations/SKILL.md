...
description: Diagnose and perform safe Git/GitHub work for roots, status, branches, commits, merges, tags, releases, remotes, pushes, and PRs.
---
...
The helper normalizes PowerShell's unquoted comma-list binding for `gh --json`
field lists before execution; use `-NormalizeOnly` for offline argument-shape
tests and do not bypass the helper for release evidence just because a manual
`gh --json a,b,c` call failed under script binding.

Use `skills/codex-git-operations/scripts/Invoke-CodexGitWorkflow.ps1` as the
single scripted Git entry:
...
Within the Global Experience Agent, Git, version, push, tag, and release are
platform specialist capabilities registered behind the `git` or `release`
tool gate. The root or a concept/subagent may request the capability, but only
`codex-git-operations` may preflight and execute it; the result, checkpoint,
publication envelope, failure report, and experience-capture handoff return to
the parent Agent save point. Delegation never turns Git authorization into a
standing permission.
...
When the calculated release tag exists only as a detached local tag from rewritten history, while the target remote and GitHub Release both lack that tag, the release controller preserves the original tag object under `refs/tags/codex-archive/` and clears the blocking name before continuing. Tags on the current history or with any remote publication evidence remain hard blockers.
