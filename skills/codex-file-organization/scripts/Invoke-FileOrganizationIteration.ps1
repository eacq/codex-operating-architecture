...
$planArgs = @{ Root = $root; ManagedRoots = $managedRoots }
if ($Phase -eq 'global-iteration') { $planArgs['OnlyUntracked'] = $true }
$plan = & (Join-Path $PSScriptRoot 'New-FileOrganizationPlan.ps1') @planArgs | ConvertFrom-Json
...
$record = [ordered]@{ schema_version=3; phase=$Phase; managed_roots=$managedRoots; candidate_scope=$plan.candidate_scope; planned_moves=$moves.Count; reference_updates=$rewrites.Count; backup='not-needed'; moved=0; result='passed'; completed_at=[DateTime]::UtcNow.ToString('o') }
if ($Apply -and $moves.Count -gt 0) {
  ...
  foreach ($rewrite in $rewrites) {
    ...
    [IO.File]::WriteAllText($rewrite.file, $content.Replace($rewrite.old, $rewrite.new), [Text.UTF8Encoding]::new($false))
  }
  ...
  [ordered]@{ schema_version=1; moves=@($moves); rewrites=@($rewrites); created_at=[DateTime]::UtcNow.ToString('o') } |
    ForEach-Object { [IO.File]::WriteAllText((Join-Path $project 'file-organization-move-manifest.json'), (($_ | ConvertTo-Json -Depth 6) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false)) }
}
if ($Apply) {
  New-Item -ItemType Directory -Force -Path $project | Out-Null
  [IO.File]::WriteAllText((Join-Path $project 'file-organization-iteration.json'), (($record | ConvertTo-Json -Depth 5) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
}
