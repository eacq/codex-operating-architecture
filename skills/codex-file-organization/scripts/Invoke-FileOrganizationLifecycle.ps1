...
if ($Apply) {
  ...
  $reviewPath = Join-Path $project 'file-organization-review.json'
  [IO.File]::WriteAllText($reviewPath, (($record | ConvertTo-Json -Depth 7) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
}
