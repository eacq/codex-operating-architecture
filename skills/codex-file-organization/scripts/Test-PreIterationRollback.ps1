...
$fixture = & (Join-Path $root 'scripts\Resolve-CodexRunRoot.ps1') -ArchitectureRoot $root -Kind tmp -ChildPath ('codex-rollback-test-' + [guid]::NewGuid().ToString('N')) -Create
...
try {
    ...
    Set-Content -LiteralPath (Join-Path $fixture 'unchanged.txt') -Value 'unchanged' -Encoding UTF8
    ...
    & git -C $fixture add -- keep.txt restore.txt unchanged.txt
    ...
    if ($snapshot.file_inventory -ne 'git-tracked-untracked-ignored' -or $snapshot.copy_engine -ne 'robocopy-filtered-tree' -or $snapshot.hash_engine -ne 'dotnet-sha256-stream' -or $snapshot.copy_threads -ne 32) { throw 'Rollback snapshot did not use the optimized privacy-filtered copy path.' }
    ...
    if ($restored.files_restored -ne 2 -or $restored.files_skipped_identical -lt 1) { throw 'Rollback did not skip the already verified identical file.' }
