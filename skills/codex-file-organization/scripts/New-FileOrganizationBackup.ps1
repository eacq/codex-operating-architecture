...
if ($Apply -and $RelativePaths.Count -eq 0 -and $isGitRoot) {
  $trackedFileCount = @(& git -C $source ls-files --cached).Count
  if ($trackedFileCount -gt 1000) {
    ...
    [ordered]@{
      ...
      file_count = $trackedFileCount
      total_bytes = $null
      protected_paths_excluded = $true
      backup_root = 'user-selected-backup-root'
      apply_performed = $true
      backup_type = 'git-bundle'
      file_inventory = 'git-ls-files-cached-fast-path'
      archive_sha256 = (Get-FileHash -LiteralPath $bundle -Algorithm SHA256).Hash.ToLowerInvariant()
