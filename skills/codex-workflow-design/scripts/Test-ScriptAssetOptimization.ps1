...
try {
    ...
    [IO.File]::WriteAllText((Join-Path $fixture '.codex\project\global-experience-iteration.json'), (([ordered]@{ step_timings = @(@{ name = 'fast check'; duration_seconds = 0.1 }, @{ name = 'create rollback snapshot'; duration_seconds = 2.5 }) } | ConvertTo-Json -Depth 4) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
