...
$timingSources = @('.codex/project/global-experience-iteration.json', '.codex/project/isolated-global-iteration.json', '.codex/project/verification-last-run.json')
...
if ($Apply) {
    ...
    [IO.File]::WriteAllText($output, (($review | ConvertTo-Json -Depth 7) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false))
