...
description: Owner-internal implementation-planning mode for accepted designs or execution-ready requirements with validation boundaries.
---
...
Apply a fact gate before a premise may drive the plan. Any claim that changes
architecture, public interfaces, compatibility, security, dependencies,
installation, release/publication behavior, data movement, or acceptance
criteria must be one of: observed repository evidence, command output, a
primary/current source, an explicit user decision, or a hypothesis with a
dedicated verification task before the dependent action. More review,
consensus, or model agreement cannot replace the missing fact.
...
6. Self-review once: trace every acceptance criterion to a task and check;
   ...
   names/interfaces, hidden authority expansion, fact-gate failures, and tasks
   that cannot be independently verified. Repair the plan or report the
   blocker.
...
authority boundary is visible, every high-impact premise passes the fact gate,
and no unsupported detail is presented as fact. If the design or evidence is
insufficient, return the precise question or evidence task instead of creating
a false-complete plan.
