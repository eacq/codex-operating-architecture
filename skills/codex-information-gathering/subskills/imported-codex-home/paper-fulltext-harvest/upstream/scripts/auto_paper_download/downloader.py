...
# Legacy tuples kept for backwards-compat with any external imports.
# Authoritative routing lives in ``auto_paper_download.publishers``.
...
def classify_publisher(doi: str) -> str | None:
    ...
    Wraps :func:`auto_paper_download.publishers.classify_publisher` and maps the
    new, expanded publisher families (24 DOI prefixes / 19 families) back to the
    four legacy labels the rest of the pipeline still keys on
    (``Wiley`` / ``Elsevier`` / ``Springer`` / ``Crossref``).

    Publishers without an API client in this bundled v0.1 (ACS / RSC / IEEE / AIP /
    IOP / APS / etc.) are routed through the ``Crossref`` bucket, which then tries
    OpenAlex/Crossref/Unpaywall in turn. For paywalled content from those families
    the agent should drop down to the browser-fallback path described in
    ``references/browser-fallback.md`` (uses OpenClaw's ``browser`` tool with
    ``profile="user"``), or upgrade to the standalone ``auto-paper-harvester``
    v0.2+ CLI which ships an in-process Playwright fallback.
    """
    from .publishers import (
        classify_publisher as _classify_publisher_info,
        family_to_legacy_publisher,
    )
    info = _classify_publisher_info(doi)
    if info is None:
        return None
