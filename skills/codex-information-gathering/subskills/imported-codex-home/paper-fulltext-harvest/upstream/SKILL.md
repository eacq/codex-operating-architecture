...
description: >-
  Batch download academic paper full-text PDF/XML from DOI lists, savedrecs exports, or Excel. Handles publisher TDM APIs that require institutional subscription, Open Access sources such as Crossref/Unpaywall/OpenAlex, and browser fallback for paywalled publishers without TDM access. Browser fallback can drive the user logged-in Chrome through the browser tool for interactive sessions, or use the standalone auto-paper-harvester CLI with Playwright for unattended bulk runs. Use when the user wants to harvest, scrape, fetch, bulk-download, or backfill missing full-text papers.
---
