...
2. Read `get_graph_schema` and `get_architecture` before making architecture
   ...
   coverage evidence.
3. Use `search_code` for policy text, documentation wording, and symbols. It
   enriches text matches with graph context and is the current supported
   graph-backed search surface.
4. Use `trace_path` for callers/callees, data-flow, and cross-service traces
   after an exact function or route name is known.
5. Open the cited source files with normal file reads before final conclusions,
...
- `search_code` finds workflow policy text and symbols through the supported
  graph-enriched text search surface.
- `trace_path` works for indexed call, data-flow, and cross-service traversal
  when an exact function name is available.
- Stale references to older MCP tools such as `search_graph`, `query_graph`,
  and `get_code_snippet` should not be used unless the callable registry for
  the current task exposes them again.
...
trace_path(project=<name>, function_name=<exact-name>, mode=calls)
```
