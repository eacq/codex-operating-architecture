---
name: codex-credential-management-provider-routing
description: Route and diagnose a privately configured OpenAI-compatible provider. Use for safe API-key diagnostics, ChatGPT-login versus API-key routing, endpoint checks, and model fallback without exposing credentials.
---

# Private OpenAI-Compatible Provider Profile

This owner-internal subskill contains no personal endpoint, model catalog, account,
...
- When CC Switch is installed and configured (`~/.cc-switch/cc-switch.db` plus
  `settings.json`), treat it as the local provider-switch authority for Codex
  live projections. A CC Switch-managed projection can use an
  OpenAI-compatible provider block with an OAuth or ChatGPT-shaped auth cache.
  Do not use the legacy `codex-provider-switch` one-to-one provider/auth
  alignment rule as an automatic repair signal under CC Switch. Inspect only
  non-secret status, and do not print `experimental_bearer_token`, API keys,
  OAuth tokens, or database credential rows.
...
```powershell
py scripts/check_provider_routing.py --base-url <profile-base-url> --api-key-env <profile-env-var> --model <profile-model>
```
...
[references/provider-routing-reference.md](references/provider-routing-reference.md) for the
...
py scripts/check_provider_routing.py --base-url https://example.test/v1 --api-key-env TEST_API_KEY --model test-model
```
