...
foreach ($needle in @('brainstorming-lite/SKILL.md', 'one concise question at a time', 'two or three feasible approaches', 'normalized requirement brief', 'does not treat brainstorming as implementation authorization', 'Stop questioning once remaining uncertainty is low-risk', 'must not autonomously route to', 'Do not auto-open this mode', 'explicit user request for brainstorming/design exploration')) {
  if (($parentText + "`n" + $text) -notmatch [regex]::Escape($needle)) { throw "Brainstorming contract is missing: $needle" }
}
$combined = $parentText + "`n" + $text
foreach ($forbidden in @('autonomously route to `brainstorming-lite`', 'may autonomously open this mode', 'should open autonomously')) {
  if ($combined -match [regex]::Escape($forbidden)) { throw "Brainstorming contract still permits automatic opening: $forbidden" }
}
