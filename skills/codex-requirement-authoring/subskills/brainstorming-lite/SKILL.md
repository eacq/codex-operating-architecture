...
description: Owner-internal design mode for ambiguous creative, feature, or system ideas that need execution-ready shape without scope creep.
---
...
Do not auto-open this mode merely because the global experience system detects
a material unresolved choice in a proposed skill, workflow, experience,
learning, or architecture change. Use it only when the user explicitly asks for
brainstorming/design exploration, or when `codex-requirement-authoring` is
already the active owner for a request and a non-discoverable choice materially
changes scope, quality, authority, safety, cost, or validation. Bounded
repairs, obvious documentation updates, validation-only checks, routine
candidate reporting, and execution-ready reversible tasks must proceed through
conversation-to-spec or the responsible execution owner instead.
