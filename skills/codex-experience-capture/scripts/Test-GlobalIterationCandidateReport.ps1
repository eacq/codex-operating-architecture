...
try {
    ...
    @('# Project Experience', '', '## Global optimization evaluates scriptification candidates', '', '- This verified lesson is not a candidate section.', '', '## Candidates', '', '- Install candidate needs a runtime check.') | Set-Content -LiteralPath (Join-Path $fixture '.codex\project\EXPERIENCE.md') -Encoding UTF8
    ...
    if ($result.candidates.summary -contains 'This verified lesson is not a candidate section.') { throw 'Candidate report collected a non-candidate heading that only contains the word candidates.' }
