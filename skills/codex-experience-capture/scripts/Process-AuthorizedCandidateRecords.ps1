...
$errors=Join-Path $root '.codex\errors'; if (Test-Path -LiteralPath $errors) { Get-ChildItem -LiteralPath $errors -Recurse -Filter 'report.json' -File | ForEach-Object { $report=Get-Content -LiteralPath $_.FullName -Raw -Encoding UTF8|ConvertFrom-Json; if ($report.status -ne 'candidate') { return }; $r=Archive $_.FullName $archiveRoot; $markdownPath=Join-Path $_.DirectoryName 'report.md'; if (Test-Path -LiteralPath $markdownPath) { Archive $markdownPath $archiveRoot | Out-Null; Remove-Item -LiteralPath $markdownPath -Force }; Remove-Item -LiteralPath $_.FullName -Force; $processed += @{source=$r;action='archived-and-cleared-candidate-error-record'} } }
