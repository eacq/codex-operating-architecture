...
function Get-CandidateBullets([string]$Path, [string]$Category) {
    ...
    foreach ($line in $lines) {
        if ($line -match '^#{1,3}\s+') {
            $active = ($line -match '^##\s+Candidates\s*$') -or ($line -match "^##\s+$([regex]::Escape($candidateChineseHeading))\s*$")
            continue
        }
...
if ($Apply) {
    ...
    [IO.File]::WriteAllText($markdownPath, (($markdown -join [Environment]::NewLine) + [Environment]::NewLine), $utf8NoBom)
}
