...
if ($Apply) { $target = Join-Path (Get-Location) '.codex\project\visual-plan.json'; [IO.File]::WriteAllText($target, (($plan | ConvertTo-Json -Depth 4) + [Environment]::NewLine), [Text.UTF8Encoding]::new($false)) }
