...
3. Organize eligible files and repair references.
   When the accepted iteration changes reusable Global Experience Agent
   structure, update `config/agent-structural-optimization-policy.json`, then
   run `agent/80-maintenance/Sync-ChildAgentStructuralOptimizations.ps1 -Apply`.
   The iteration is not structurally complete until the minimal template, all
   registered child profiles, Agent manifest, generated projections, and
   delegation snapshot gate share the same baseline version and hash.
...
For publication-level Git work, a current complete replacement proof may be
reused only when `HEAD`, staged path hash, complete mode, rollback readiness,
post-replacement validation, lifecycle writeback, and the generated
publication envelope all match the current staged publication surface. A proof
miss reruns the complete replacement; candidate-only proof is still rejected.
