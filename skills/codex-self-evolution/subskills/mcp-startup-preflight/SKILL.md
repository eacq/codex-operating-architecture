...
1. Before checking the task root, run
   `F:\codex\scripts\Start-CodebaseMemoryGraphUi.ps1 -RepositoryRoot F:\codex -ProjectName F-codex -Port 9749`.
   ...
   remains available at `http://127.0.0.1:9749/` with `F-codex` visible.
2. Always determine whether `codebase-memory-mcp` is callable in the current
   ...
   This is the automatic structural index of the Global Experience Agent and
   applies even when the current task is not in `F:\codex`. Record the result
   as global-experience startup evidence.
   Keep the MCP auto-index setting disabled. Automatic indexing is limited to
   the explicit F-codex refresh above; do not let the MCP discover or index
   the C-drive Codex installation, Codex home, Agent home, or system roots.
   Inspect or index a C-drive path only when the current task explicitly
   requires host-implementation evidence, then use a temporary task-scoped
   project and delete it before the single-project validation gate.
...
4. When the current project is a source repository other than `F:\codex`, call
   `index_repository` only when source-structure evidence is relevant to the
   current task, using the exact callable-contract key `repo_path` (not
   `repository_path`) and:
   ...
   For the canonical Agent repository, use the single stable project name
   `F-codex`. Do not create additional aliases such as `codex` or
   `codex-operating-architecture` for the same `F:\codex` root. External
   reference repositories should usually be inspected with local files or
   indexed only in short-lived task evidence; do not leave them in the global
   MCP cache when `Test-CodebaseMemoryProjectScope.ps1` is expected to pass.
...
The current verification target is not a fixed node or edge count. It is:
`index_repository(repo_path="F:\\codex", mode=fast, name="F-codex")` returns
`status=indexed`, the graph UI reports `F-codex` healthy, and
`scripts/Test-CodebaseMemoryProjectScope.ps1` shows exactly one cached project:
`F-codex` rooted at `F:/codex`. Node and edge counts are evidence for the
current run, not stable documentation constants.
