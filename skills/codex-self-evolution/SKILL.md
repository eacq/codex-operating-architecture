...
4. At Git milestones or completed iterations, reconcile project requirements, workflows, experience, retrospectives, pending events, state, and file organization. Route initialization and material follow-up through `codex-file-organization`: it automatically backs up and safely processes the managed inbox, analyzes supported references/configuration, and validates before accepting a move.
...
Before every Git commit, push, tag, release, or remote update, complete `scripts/Invoke-CompleteGlobalExperienceIteration.ps1 -Staged -Apply`, then run `scripts/Test-ExperienceIterationGate.ps1 -Staged -Apply` after staging. The complete iteration must run the architecture in an isolated copy through backup, all-file organization, reference repair, Git-layout restoration, full validation, and validated replacement. The publication gate only verifies that replacement proof and must reject a missing, incomplete, stale, or staged-path-mismatched proof; it must never reorganize the active repository directly. Before economy/refinement, test the integration contracts among affected skills, knowledge, experience, workflows, file organization, backup readiness, and visuals; use the result to improve robustness before removing, merging, or simplifying any step.
