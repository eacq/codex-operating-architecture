...
4. At Git milestones or completed iterations, reconcile project requirements, workflows, experience, retrospectives, pending events, and state.
...
Before every Git commit, push, tag, release, or remote update, complete `scripts/Invoke-CompleteGlobalExperienceIteration.ps1 -Staged -Apply`, then run `scripts/Test-ExperienceIterationGate.ps1 -Staged -Apply` after staging. The publication gate must reject a stale proof or a proof for different staged paths. Before economy/refinement, test the integration contracts among affected skills, knowledge, experience, and workflows; use the result to improve robustness before removing, merging, or simplifying any step.
