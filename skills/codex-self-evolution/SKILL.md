...
Before every Git commit, push, tag, release, or remote update, complete `scripts/Invoke-CompleteGlobalExperienceIteration.ps1 -Staged -Apply`, then run `scripts/Test-ExperienceIterationGate.ps1 -Staged -Apply` after staging. The global iteration invokes the file-organization lifecycle probe and rejects a missing policy, while preserving aggregate-only results. The publication gate must reject a stale proof or a proof for different staged paths. Before economy/refinement, test the integration contracts among affected skills, knowledge, experience, workflows, file organization, backup readiness, and visuals; use the result to improve robustness before removing, merging, or simplifying any step.
