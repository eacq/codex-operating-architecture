...
Before every Git commit, push, tag, release, or remote update, complete `scripts/Invoke-CompleteGlobalExperienceIteration.ps1 -Staged -Apply`, then run `scripts/Test-ExperienceIterationGate.ps1 -Staged -Apply` after staging. The complete iteration must run the architecture in an isolated copy through backup, all-file organization, reference repair, Git-layout restoration, quarantined evidence-based cleanup, full sandbox validation, validated replacement, two active-repository validations, real global-interface validation, and lifecycle writeback. The publication gate only verifies that complete proof and must never reorganize or clean the active repository directly. Before economy/refinement, test the integration contracts among affected skills, knowledge, experience, workflows, file organization, backup readiness, and visuals; use the result to improve robustness before removing, merging, deleting, or simplifying any step.
