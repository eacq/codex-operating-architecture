...
if ($Apply) {
    $json = ($review | ConvertTo-Json -Depth 4) + [Environment]::NewLine
    [IO.File]::WriteAllText((Join-Path $root '.codex\project\workflow-error-review.json'), $json, [Text.UTF8Encoding]::new($false))
}
