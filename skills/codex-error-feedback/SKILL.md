...
description: Turn failures, wrong results, missing behavior, slow workflows, and repeated mistakes into redacted repair reports and verified lessons.
---
...
## Transport failure fast-stop

For `codex`, the Global Experience Agent, specialist/concept Agents, and child
Agents, use `config/agent-transport-recovery-policy.json` and
`agent/40-runtime/Get-AgentTransportRecoverySignal.ps1` when a stream, TLS,
connection, or request-timeout failure occurs before completion. The runtime
must stop automatic retry, persist the durable state and pending writes, return
the typed `restart-required` exit, and tell the user to restart Codex or the
affected host. `Resume` is the recovery boundary; `Continue` must not silently
retry a `blocked-restart-required` session. Ordinary validation, parser,
authorization, and business errors remain on the normal error path.

When the native Codex host exposes a thread snapshot but no error text, use
`agent/40-runtime/Get-AgentHostRecoverySignal.ps1` with the result of
`codex_app__list_threads` plus `codex_app__wait_threads`. An active
`inProgress` turn with no assistant/tool marker, or with a stale `inProgress`
tool marker beyond the policy threshold, is `host-stall-suspected`: persist the host-recovery marker, prompt
the user to restart, and never force-kill the task. This host watchdog is the
bridge for silent hangs that never reach the Agent runtime `catch` block.
