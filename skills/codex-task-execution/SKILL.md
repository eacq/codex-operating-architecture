...
description: Execute scoped technical or operational tasks through implementation, commands, artifact creation, verification, and handoff.
---
...
When inserting an independent validation section into a script or test, keep
resource-disposal `try`/`finally` blocks structurally adjacent. Parse the edited
file before behavior tests so a misplaced validation block cannot detach
cleanup from the resource scope it protects.
...
Inside the Global Experience Agent, an existing owner is a specialist Agent and
a multi-owner profile is a concept Agent. Create a child only through the
registered delegate operation, then require evidence-bearing completion and an
explicit accepted/rejected join decision from this merge owner. Cancellation
settles child state without merging it. No child may widen authority or bypass
the parent Agent's owner tool gates.
