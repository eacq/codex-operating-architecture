...
if (Test-Path -LiteralPath $profilePath) {
    $portableProfile = Get-Content -LiteralPath $profilePath -Raw -Encoding UTF8 | ConvertFrom-Json
}
$softwareInstallRoot = if ($env:SOFTWARE_INSTALL_ROOT) { $env:SOFTWARE_INSTALL_ROOT } elseif ($portableProfile -and $portableProfile.software -and $portableProfile.software.install_root) { $portableProfile.software.install_root } else { '' }
...
function Resolve-CommandPath([string]$Command) {
    if ([IO.Path]::IsPathRooted($Command)) { return $Command }
    $resolved = Get-Command $Command -ErrorAction SilentlyContinue
    if ($resolved) { return $resolved.Source }
    return $Command
}

$condaPath = Resolve-CommandPath $conda

function Assert-Conda {
    if (-not (Test-Path -LiteralPath $condaPath)) { throw "Miniconda not found at $conda. Route installation through codex-tool-installation." }
}
...
function Invoke-ProjectPipInstall([string]$Python, [string[]]$Requested) {
    $previousNoUserSite = $env:PYTHONNOUSERSITE
    try {
        $env:PYTHONNOUSERSITE = '1'
        $output = & $Python -m pip install --no-user --upgrade --force-reinstall @Requested 2>&1
...
function Update-ProjectPipLock([string]$Python, [string]$LockPath) {
    $previousNoUserSite = $env:PYTHONNOUSERSITE
    try {
        $env:PYTHONNOUSERSITE = '1'
        & $Python -m pip freeze | Set-Content -LiteralPath $LockPath -Encoding UTF8
    } finally {
        if ($null -eq $previousNoUserSite) {
            Remove-Item Env:\PYTHONNOUSERSITE -ErrorAction SilentlyContinue
        } else {
...
function Initialize-Project([string]$Root) {
    ...
    if (-not (Test-Path -LiteralPath $p.Prefix)) {
        ...
        & $condaPath create --yes --prefix $p.Prefix --clone $basePrefix
    ...
    Update-ProjectPipLock $python $p.Lock
...
switch ($Action) {
    'status' {
        ...
        [pscustomobject]@{ project_root = $p.Root; base_ready = Test-Path (Join-Path $basePrefix 'python.exe'); project_ready = Test-Path (Join-Path $p.Prefix 'python.exe'); manifest = Test-Path $p.Manifest; conda = Test-Path $condaPath; conda_path = $condaPath }
    }
    'init-base' {
        Assert-Conda
        if (-not (Test-Path -LiteralPath $basePrefix)) {
            & $condaPath env create --prefix $basePrefix --file (Join-Path $catalogRoot 'base\environment.yml')
    ...
    'add-profile' {
        ...
        if ($Apply) {
            ...
            $python = Join-Path $p.Prefix 'python.exe'
            $requirements = @($profileObject.packages | ForEach-Object { $_.requirement })
            $exitCode = Invoke-ProjectPipInstall $python $requirements
            if ($exitCode -ne 0) {
                foreach ($item in @($profileObject.packages)) {
                    Record-Event $p.Root $item.package $item.requirement "profile:$($profileObject.name); $($item.capability)" 'failed' 'pip install'
                }
                throw "Dependency installation failed for profile: $($profileObject.name)"
            }
            foreach ($item in @($profileObject.packages)) {
    ...
    'add' {
        ...
        if ($Apply) {
            if (-not (Test-Path (Join-Path $p.Prefix 'python.exe'))) { Initialize-Project $ProjectRoot }
            $python = Join-Path $p.Prefix 'python.exe'
            $exitCode = Invoke-ProjectPipInstall $python @($requested)
            if ($exitCode -ne 0) { Record-Event $p.Root $Package $requested $Purpose 'failed' 'pip install'; throw 'Dependency installation failed.' }
            Update-ProjectPipLock $python $p.Lock
        }
