...
DEFAULT_OPENAI_MODEL = os.environ.get("ZCHAT_MODEL", "gemini-3-pro")
DEFAULT_OPENAI_BASE_URL = os.environ.get("ZCHAT_BASE_URL", "https://api.zchat.tech/v1")
...
class TransitionPromptGenerator:
    ...
    def __init__(self, template_path: str = DEFAULT_TEMPLATE_PATH) -> None:
        ...
        self.provider, self.client = self._init_model_client()
        print(f"{self.provider} API client initialized")

    def _init_model_client(self) -> Tuple[str, Any]:
        """
        Initialize model client.

        Returns:
            Tuple of provider name and configured client.

        Raises:
            PromptGeneratorError: If initialization fails.
        """
        preferred_provider = os.environ.get("PPT_TRANSITION_PROVIDER", "").lower()
        zchat_key = os.environ.get("ZCHAT_API_KEY") or os.environ.get("OPENAI_API_KEY")
        if preferred_provider != "anthropic" and zchat_key:
            if OpenAI is None:
                raise PromptGeneratorError(
                    "OpenAI library is not installed. Run: pip install openai"
                )
            return (
                "ZCHAT/OpenAI-compatible",
                OpenAI(api_key=zchat_key, base_url=DEFAULT_OPENAI_BASE_URL),
            )
        ...
        try:
            ...
            raise PromptGeneratorError(
                f"Model API initialization failed.\n"
                f"Set ZCHAT_API_KEY for ZCHAT/OpenAI-compatible mode,\n"
                f"or set ANTHROPIC_API_KEY for Anthropic mode.\n"
                f"Error: {e}"
            )

    @staticmethod
    def _to_openai_content(message_content: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        openai_content = []
    ...
    def _call_model(self, message_content: List[Dict[str, Any]], max_tokens: int) -> str:
        if self.provider == "ZCHAT/OpenAI-compatible":
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": self._to_openai_content(message_content),
                    }
                ],
            )
...
"""
        ...
        print(f"Calling {self.provider} API for transition analysis...")

        try:
            transition_prompt = self._call_model(message_content, DEFAULT_MAX_TOKENS)
            ...
            raise PromptGeneratorError(f"{self.provider} API call failed: {e}")
...
Please describe this subtle animation in one paragraph (150-250 words).""",
        ...
        print(f"Calling {self.provider} API for preview prompt...")

        try:
            preview_prompt = self._call_model(message_content, 1000)
