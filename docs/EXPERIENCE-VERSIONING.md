...
- `R` is determined only by the latest private GitHub Release. Private releases use tags such as `v1.0`; the first private baseline under public major `1` is `v1.0`.
...
- `R` 仅由最新私有 GitHub Release 决定。私有 release 使用 `v1.0` 形式标签；公开主版本为 `1` 时，首次私有基线为 `v1.0`。
